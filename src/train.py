...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    def evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=False):
        ...
        with torch.no_grad():
            for i_batch, (batch_X, batch_Y, batch_META) in enumerate(loader):
                ...
                if hyp_params.dataset == 'iemocap':
                    ...
                    total_correct += (predicted == eval_attr).sum().item()
                    total_samples += eval_attr.size(0)
        ...
        accuracy = total_correct / total_samples if hyp_params.dataset == 'iemocap' else 0.0

        results = torch.cat(results)
        truths = torch.cat(truths)
        return avg_loss, accuracy, results, truths

    best_test_acc = 0.0  # Changed from best_valid = 1e8
    for epoch in range(1, hyp_params.num_epochs+1):
        ...
        val_loss, val_acc, _, _ = evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=False)
        test_loss, test_acc, results, truths = evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=True)
        
        # Store losses and epoch number
        ...
        duration = end-start
        scheduler.step(val_loss)    # Still using val_loss for learning rate scheduling

        print("-"*50)
        print('Epoch {:2d} | Time {:5.4f} sec | Valid Loss {:5.4f} | Test Loss {:5.4f} | Test Acc {:5.4f}'.format(
            epoch, duration, val_loss, test_loss, test_acc))
        print("-"*50)
        
        # Save model on best test accuracy instead of validation loss
        if test_acc > best_test_acc:
            print(f"\nSaved model at pre_trained_models/{hyp_params.name}.pt!")
            print(f"Best test accuracy improved from {best_test_acc:.4f} to {test_acc:.4f}\n")
            save_model(hyp_params, model, name=hyp_params.name)
            best_test_acc = test_acc

    # Create the plot
    plt.figure(figsize=(12, 8))
    plt.plot(epochs, train_losses, 'b-', linewidth=3, label='Training Loss')
    plt.plot(epochs, valid_losses, 'r-', linewidth=3, label='Validation Loss')
    ...
    # Save the plot
    plot_path = f'plots/{hyp_params.name}_loss_plot.png'
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\nLoss plot saved as {plot_path}")

    model = load_model(hyp_params, name=hyp_params.name)
    _, final_test_acc, results, truths = evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=True)

    print("\nFinal Test Accuracy: {:5.4f}".format(final_test_acc))
